(() => {
  "use strict";
  const app = document.getElementById("nefro-tema-app");
  ...
  document.title = `${tema.titulo} | Nefrología | BADBEAR.MED`;
  ...
  app.innerHTML = `
    <div class="tema-breadcrumb">
      <a href="../index.html">BADBEAR.MED</a><span>›</span><a href="index.html">Nefrología</a><span>›</span><strong>${tema.titulo}</strong>
    </div>
    <section class="tema-hero">
      <div class="tema-icon">${tema.icono}</div>
      <div><span>${tema.area.toUpperCase()} · NEFROLOGÍA</span><h1>${tema.titulo}</h1><p>${tema.descripcion}</p></div>
    </section>
      ...
      <div>
        <section class="tema-card">
          ...
          <ul>
            ...
            <li>Fisiopatología y correlación clínica.</li>
            <li>Hallazgos de laboratorio y sedimento urinario cuando corresponda.</li>
            <li>Diagnóstico, clasificación y diagnóstico diferencial.</li>
  ...
  document.getElementById("tema-complete")?.addEventListener("click", (e) => {
    ...
    localStorage.setItem("badbear_nefro_leidos", JSON.stringify([...leidos]));
