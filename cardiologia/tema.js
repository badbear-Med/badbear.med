(() => {
  "use strict";
  const app = document.getElementById("cardio-tema-app");
  const temas = Array.isArray(window.BADBEAR_CARDIO_TEMAS) ? window.BADBEAR_CARDIO_TEMAS : [];
  ...
  app.innerHTML = `
    <div class="tema-breadcrumb">
      <a href="../index.html">BADBEAR.MED</a><span>›</span><a href="index.html">Cardiología</a><span>›</span><strong>${tema.titulo}</strong>
    </div>
    <section class="tema-hero">
      <div class="tema-icon">${tema.icono}</div>
      <div><span>${tema.area.toUpperCase()} · CARDIOLOGÍA</span><h1>${tema.titulo}</h1><p>${tema.descripcion}</p></div>
    </section>
      ...
      <div>
        <section class="tema-card">
          ...
          <ul>
            ...
            <li>Fisiopatología y factores de riesgo.</li>
            <li>Manifestaciones clínicas y hallazgos del examen.</li>
            <li>Electrocardiografía, laboratorio e imagen cuando corresponda.</li>
            <li>Diagnóstico diferencial y criterios de gravedad.</li>
            <li>Tratamiento, seguimiento y puntos de alta rentabilidad.</li>
          </ul>
        </section>
        <section class="tema-card">
  ...
  document.getElementById("tema-complete")?.addEventListener("click", (e) => {
    ...
    localStorage.setItem("badbear_cardio_leidos", JSON.stringify([...leidos]));
