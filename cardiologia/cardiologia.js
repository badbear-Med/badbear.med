(() => {
  ...
  const grid = document.getElementById("cardio-temas");
  const buscar = document.getElementById("cardio-buscar");
  const filtro = document.getElementById("cardio-filtro");
  const totalTemas = document.getElementById("cardio-total-temas");
  const totalAreas = document.getElementById("cardio-total-areas");
  const completados = document.getElementById("cardio-completados");
  const porcentaje = document.getElementById("cardio-porcentaje");
  ...
  function tarjeta(t){
    ...
    a.innerHTML = `
      <div class="cardio-topic-top">
        ...
        <span class="cardio-topic-area">${t.area}</span>
      </div>
      <h3>${t.titulo}</h3>
      <p>${t.descripcion}</p>
