(() => {
  "use strict";
  const app = document.getElementById("psiq-tema-app");
  ...
  document.title = `${tema.titulo} | Psiquiatría | BADBEAR.MED`;
  ...
  app.innerHTML = `
    <div class="tema-breadcrumb">
      <a href="../index.html">BADBEAR.MED</a><span>›</span><a href="index.html">Psiquiatría</a><span>›</span><strong>${tema.titulo}</strong>
    </div>
    <section class="tema-hero">
      <div class="tema-icon">${tema.icono}</div>
      <div><span>${tema.area.toUpperCase()} · PSIQUIATRÍA</span><h1>${tema.titulo}</h1><p>${tema.descripcion}</p></div>
    </section>
    <div class="tema-layout">
      <div>
        <section class="tema-card">
          ...
          <ul>
            <li>Definición y criterios diagnósticos esenciales.</li>
            <li>Presentación clínica y examen mental.</li>
            <li>Diagnóstico diferencial y comorbilidades.</li>
            <li>Evaluación de gravedad y riesgo.</li>
            <li>Tratamiento farmacológico y no farmacológico.</li>
  ...
  document.getElementById("tema-complete")?.addEventListener("click", (e) => {
    ...
    localStorage.setItem("badbear_psiquiatria_leidos", JSON.stringify([...leidos]));
